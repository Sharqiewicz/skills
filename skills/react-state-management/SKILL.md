...
Expert guidance for categorising and managing React state using the right tool for each state type — server, global client, injected, or event-driven.

## MANDATORY PREPARATION

1. Identify which state type and tool the question involves: React Query (server state), Zustand (global client), Context (DI), XState (event-driven flows), or a combination.
2. Read the relevant reference file before answering:
   - Server/async state → [react-query.md](react-query.md)
   - Global client state → [zustand.md](zustand.md)
   - Context / dependency injection → [react-context.md](react-context.md)
   - Event-driven or complex flows → [xstate.md](xstate.md)
   - Cross-tool patterns → [patterns.md](patterns.md)
3. If reviewing existing code, read the code under review before suggesting changes.
...
**CRITICAL**: Categorising state incorrectly — e.g. putting server state in Zustand — creates a second source of truth and breaks cache invalidation. Always categorise before choosing an API.
...
**IMPORTANT**: Context + useState is not a state management solution. When components need to update shared state, use Zustand or React Query — not a Context wrapped around a useState.
...
| `onSuccess` callback used to sync to other state | Removed in current React Query; use data directly or `select` |
...
**NEVER:**
- Recommend storing server state in Zustand — it creates a second source of truth and breaks cache invalidation
- Suggest `useEffect` to sync server state into local state — derive at render time instead
- Recommend exporting the raw Zustand store — export typed selector hooks only
- Suggest Context + useState as a state management pattern — use Zustand or React Query for mutable shared state
- Recommend calling `refetch()` with arguments — update the state that drives the query key instead
- Mix state types in one tool — categorise first, then choose the correct tool
...
After providing guidance, confirm:
- [ ] State was correctly categorised before a tool was recommended
- [ ] The relevant reference file was read before answering
- [ ] No recommendation contradicts the NEVER block above
- [ ] Code examples are consistent with the Common Mistakes table
...
State categorisation is the first decision — choosing the wrong category multiplies complexity across the entire codebase, so get that right before picking an API.
